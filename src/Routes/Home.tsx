...
function Home() {
  ...
  const nowPlayingData = useQuery<IExtendsResult>(
    ["movies", "nowPlaying"],
    getNowPlayingMovies
  );
  const popularData = useQuery<IMoviesResult>(
    ["movies", "popular"],
    getPopularMovies
  );
  ...
  const nowPlayingIncreaseIndex = () => {
    if (nowPlayingData.data) {
      ...
      toggleNowLeaving();
      const totalMovies = nowPlayingData.data?.results.length - 1;
  ...
  const popularIncreaseIndex = () => {
    if (popularData.data) {
      ...
      const totalMovies = popularData.data?.results.length;
      const maxIndex = Math.floor(totalMovies / offset) - 1;
  ...
  const topRatedIncreaseIndex = () => {
    if (topRatedData.data) {
      ...
      const totalMovies = topRatedData.data?.results.length;
      const maxIndex = Math.floor(totalMovies / offset) - 1;
    ...
    <Wrapper>
      {isLoading ? (
        ...
        <>
          ...
          >
            <Title>{nowPlayingData.data?.results[0].title}</Title>
            ...
            {bigMovieMatch ? (
              <>
                ...
                <BigMovie
                  ...
                  layoutId={
                    bigMovieMatch.params.movieId + bigMovieMatch.params.category
                  }
                >
                  {clickedMovie && (
                    <>
